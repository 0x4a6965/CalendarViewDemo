...
public class DirectionalViewPager extends ViewPager {
	...
	private boolean mScrollable; //是否开启滑动
...
//	}
	...
	protected void addNewItem(int position, int index) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mChildWidthMeasureSpec = View.MeasureSpec.makeMeasureSpec(getMeasuredWidth() -
				getPaddingLeft() - getPaddingRight(), View.MeasureSpec.EXACTLY);
		mChildHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(getMeasuredHeight() -
				getPaddingTop() - getPaddingBottom(), View.MeasureSpec.EXACTLY);
		...
		for (int i = 0; i < size; ++i) {
			...
			if (child.getVisibility() != GONE) {
				if (DEBUG)
					Log.v(TAG, "Measuring #" + i + " " + child + ": " + mChildWidthMeasureSpec + " x " + mChildHeightMeasureSpec);
				child.measure(mChildWidthMeasureSpec, mChildHeightMeasureSpec);
			}
		}
	}
		...
		switch (action & MotionEventCompat.ACTION_MASK) {
			...
			case MotionEvent.ACTION_UP:
				if (mIsBeingDragged) {
					...
					if ((Math.abs(initialVelocity) > mMinimumVelocity) || Math.abs(mInitialMotion - lastMotion) >= sizeOverThree) {
